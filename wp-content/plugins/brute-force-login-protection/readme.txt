...
== Installation ==
1. Install the plugin either via the CMS.org plugin directory, or by uploading the files to your wp-content/plugin directory.
2. Activate the plugin through the CMS admin panel.
...
If you don't have FTP access, the only way to unblock your IP is to log in your CMS admin panel from another IP address and unblock it via the plugin settings page.

= I get an error: .htaccess file not readable/writeable =
Brute Force Login Protection will only work if your .htaccess file is writeable by CMS. If you get this error, make sure that your .htaccess file has read and write permissions.
