...
if ( ! defined( 'MPC_MASSIVE_VERSION' ) ) {
	define( 'MPC_MASSIVE_VERSION', '2.3.3' );
}
...
if ( ! defined( 'MPC_MASSIVE_PREVIEW_URL' ) ) {
	define( 'MPC_MASSIVE_PREVIEW_URL', 'http://massive.mpcthemes.net/' );
}
...
if ( ! function_exists( 'mpc_backend_enqueue' ) ) {
	function mpc_backend_enqueue() {
		...
		$vc_enabled_post_types   = vc_editor_post_types();
		$vc_enabled_post_types[] = 'templatera';

		if ( ! ( is_object( $admin_page ) && in_array( $admin_page->post_type, $vc_enabled_post_types ) ) ) {
			return;
		}
