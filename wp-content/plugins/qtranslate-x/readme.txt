...
**Very Important**: Whenever you update the plugin, make sure to deactivate the previous version and then activate the new one. Normal CMS update does that, and should be sufficient, but if you overwrote plugin files manually, be sure to execute deactivation/activation cycle, otherwise you will miss the execution of activation hooks and some options may become misconfigured. for For the sake of performance, plugin is not programmed to run all the necessary checks every time it is loaded, since activation hook is an expensive operation. That is why it is important to execute deactivation/activation cycle.

Otherwise the installation is similar to any other CMS plugin:

1. Download the plugin from [CMS](http://wordpress.org/plugins/qtranslate-x/ "qTranslate-X") or take the latest development version from [GitHub](https://github.com/qTranslate-Team/qtranslate-x).
1. Use CMS `/wp-admin/plugin-install.php` page to install a plugin or extract all the files and upload everything (keeping the directory structure) to the `/wp-content/plugins/` directory.
...
1. Activate qTranslate-X through the 'Plugins' (`/wp-admin/plugins.php`) configuration page in CMS.
...
* Fix: Predefined locales are changed to match [CMS locales](https://make.wordpress.org/polyglots/teams/):<br>
Estonian (Eesti) 'et_EE' renamed to 'et',<br>
Basque (Euskera, in native alphabet, Euskara, in CMS, both correct) eu_ES renamed to 'eu',<br>
...
### 3.4.5.4 ###
* Feature: Type 'custom' with arbitrary format of items for widget "qTranslate Language Chooser". Arguments of function `qtranxf_generateLanguageSelectCode` [are chenge](https://qtranslatexteam.wordpress.com/faq/#LanguageSwitchingMethods346) to comply with CMS standards. Compatibility with old arguments is preserved.
...
* Fix: security exploit found by CMS for vulnerable parameters `json_config_files` and `json_custom_i18n_config`: [report](https://qtranslatexteam.wordpress.com/2015/09/03/why-qtranslate-x-disappeared-from-wordpress-repository).
...
* Fix: locale 'ja_JP' changed back to 'ja' as this is what CMS uses. Files lang/qtranslate-ja_JP.mo/po renamed to qtranslate-ja.mo/po.
...
* Feature: after the first activation, the current CMS active language becomes the default one for qTranslate-X, instead of English. No other languages are added anymore.
