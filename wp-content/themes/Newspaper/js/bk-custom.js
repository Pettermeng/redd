...
function functionUploadFile(fileName, labelName) {
    ...
    if(fileName == 'upload_document'){
        var input = '<input type="file" class="form-control-file " id="upload_document_'+i+'" name="upload_document[]" accept=".docx,.pdf,.xlsx,.pptx" >';
    }else if(fileName == 'upload_kml'){
        var input = '<input type="file" class="form-control-file " id="upload_document_'+i+'" name="upload_kml[]" accept=".kmz,.kml,.shp,.shx,.dbf" >';
    }else if(fileName == 'upload_jpg'){
        var input = '<input type="file" class="form-control-file " id="upload_document_'+i+'" name="upload_jpg[]" accept=".jpg,.png,.svg" >';
    }else if(fileName == 'upload_optional'){
        var input = '<input type="file" class="form-control-file " id="upload_document_'+i+'" name="upload_optional[]" accept=".docx,.pdf,.xlsx,.pptx" >';
    }else if(fileName == 'upload_mrv'){
        var input = '<input type="file" class="form-control-file " id="upload_document_'+i+'" name="upload_mrv[]" accept=".docx,.pdf,.xlsx,.pptx" >';
    }else if(fileName == 'upload_safeguard'){
        var input = '<input type="file" class="form-control-file " id="upload_document_'+i+'" name="upload_safeguard[]" accept=".docx,.pdf,.xlsx,.pptx" >';
    }else if(fileName == 'upload_benefit'){
        var input = '<input type="file" class="form-control-file " id="upload_document_'+i+'" name="upload_benefit[]" accept=".docx,.pdf,.xlsx,.pptx" >';
    }
    // console.log(input);
    // multiple
    ...
    document.getElementById('upload_document_'+i).addEventListener("change", function (data) {
        ...
        if (files.length > 0) {
            ...
            Array.from(files).forEach((file) => {
                ...
                // var input = '<input type="file" class="form-control-file hiddenFile" id="upload_document_'+p+'" name="upload_documentFile[]" accept=".docx,.pdf,.xlsx,.pptx">';
...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        //Validate project unique name
        $("#project-name").change(function() {
            var projectNameList = JSON.parse($("#list-project-name").val());
            var projectNameValue = $("#project-name").val().toLowerCase();
            $.each( projectNameList, function( key, value ) {
                if (value.project_name.toLowerCase() === projectNameValue) {
                    $("#project-name-duplicate-label").css("opacity", "1");
                    $("#next-step").prop('disabled', true).css("opacity","0.5");
                    return false;
                }else{
                    $("#project-name-duplicate-label").css("opacity", "0");
                    $("#next-step").prop('disabled', false).css("opacity","1");
                }
            });
        });
